import { Accordion, Flex, Paper, Text, Image, Stack } from '@mantine/core';
import { useMediaQuery } from '@mantine/hooks';


export default function SkillsAccordion() {
    const isVSm = useMediaQuery('(min-width: 520px)');

    return (
      <Accordion defaultValue="Backend" chevronIconSize={24}>
        <Accordion.Item key={"Backend"} value={"Backend"}>
          <Accordion.Control c={"white"} fz={24}
            style={({
                backgroundColor: 'transparent',
            })}
          >Backend</Accordion.Control>
          <Accordion.Panel>
            <Flex gap={24} justify="center" align={isVSm ? 'stretch' : "center"} wrap={"wrap"} direction={{ base: 'row', sm: 'row' }}>
              <Paper shadow="xl" c="black" p="xl" maw={"200px"}>
                <Stack justify='center' align='center' h={"100%"}>
                  <Image src="/nodejs.png" alt="Node.js Logo"/>
                  <Text fz={18}>Node.js</Text>
                </Stack>
              </Paper>
                <Paper shadow="xl" c={"black"} p="xl" maw={"200px"}>
                  <Stack justify='center' align='center' h={"100%"}>
                    <svg height="auto" viewBox="-.9 2.3 623.3 616.8" width="fit" xmlns="http://www.w3.org/2000/svg"><path d="m293.5 3.1c-51 2.5-97.7 16.6-140.4 42.3-23.9 14.4-52.3 38.1-71.2 59.1-56.9 63.6-82.8 136.8-78.8 223 2.4 51.9 16.3 98.1 42.4 141.5 30.8 51.1 82.6 98.2 134.7 122.4 41 19.1 81.8 27.7 130.8 27.7 64.5 0 118.6-15.8 168.5-49.2 30.7-20.5 57.1-45.7 80.1-76.2 43.1-57.4 62.8-123.5 59.3-199.2-2-42.7-10.9-78.9-28.4-115.2-22.3-46.3-54.9-85.6-97.7-117.5-46.4-34.8-102.5-55.4-158.3-58.3-22.6-1.1-26.3-1.2-41-.4zm-73.8 190.8c16.5 4.1 30.9 12.7 43.5 25.9 18.6 19.5 28.9 44.4 32.7 78.4.6 5.4 1.1 12.4 1.1 15.8v6h-172.3l.6 11.2c1.8 33.1 18.7 55.5 48.4 64 10.3 3 32.3 3.2 42.3.5 19.3-5.2 35.8-20.3 38.6-35.3l.6-3.4h38l-.8 3.2c-5.7 26.4-22.4 47.7-47.1 60.2-17.3 8.7-44.4 12.5-68 9.6-24-3-42.6-11.6-58.4-27-16.3-16-25.4-34.2-31.2-62.5-2.9-14.6-3.2-46.5-.5-59.1 6.7-30.9 23.2-57.1 45.6-72.3 12.2-8.3 29-15 42.7-17.1 10-1.5 34.8-.5 44.2 1.9zm171.2 38.6c15.6 22.8 28.7 41.5 29.2 41.5.4 0 13.9-18.5 30-41.1s29.7-41.3 30.3-41.5 11.4-.3 23.9-.2l22.8.3-39.7 53c-21.8 29.1-40.3 54-41.1 55.2-1.4 2.1.7 5.2 43.9 65.5 25 34.8 45.8 63.9 46.2 64.5.6 1-4.6 1.3-24.6 1.3l-25.3-.1-32.9-48.9c-18-27-33.1-49-33.5-49s-15.6 22-33.7 49l-32.9 48.9-23.7.1c-13.1 0-23.8-.2-23.8-.4 0-.3 20-28.5 44.5-62.8s44.3-62.8 44.1-63.4c-.3-.7-18.7-26-41-56.4s-40.6-55.6-40.6-56.1 10.9-.9 24.8-.9h24.7z"/><path d="m181 225c-14.6 1.9-25.4 7.3-36.1 18-6.3 6.1-8.4 9.2-12.2 17-4.3 8.9-7.7 20.7-7.7 26.4v2.6h131v-3.3c0-5.9-3.8-18.5-8-26.6-9.7-18.7-27.3-31.1-48.4-34-8.4-1.2-10.4-1.2-18.6-.1z"/></svg>
                    <Text fz={18}>Express</Text>
                  </Stack>
                </Paper>
                <Paper shadow="xl" c="black" p="xl" maw={"200px"}>
                  <Stack justify='center' align='center' h={"100%"}>
                    <Image w={"100%"} src="https://www.freelogovectors.net/wp-content/uploads/2023/09/mongodb_logo-freelogovectors.net_.png" alt="MongoDB Logo | 01"/>
                    <Text fz={18}>MongoDB</Text>
                  </Stack>
                </Paper>
                <Paper shadow="xl" c={"black"} p="xl" maw={"200px"}>
                  <Stack justify='center' align='center' h={"100%"}>
                    <svg height="auto" preserveAspectRatio="xMidYMid" viewBox="-3.576 -8.996 259.406 260.1" width="fit" xmlns="http://www.w3.org/2000/svg"><g fill="#00546b"><path d="m235.648 194.212c-13.918-.347-24.705 1.045-33.752 4.872-2.61 1.043-6.786 1.044-7.134 4.35 1.392 1.392 1.566 3.654 2.784 5.567 2.09 3.479 5.741 8.177 9.047 10.614 3.653 2.783 7.308 5.566 11.134 8.002 6.786 4.176 14.442 6.611 21.053 10.787 3.829 2.434 7.654 5.568 11.482 8.177 1.914 1.39 3.131 3.654 5.568 4.523v-.521c-1.219-1.567-1.567-3.828-2.784-5.568-1.738-1.74-3.48-3.306-5.22-5.046-5.045-6.784-11.308-12.7-18.093-17.571-5.567-3.828-17.747-9.047-20.008-15.485 0 0-.175-.173-.348-.347 3.827-.348 8.35-1.74 12.005-2.784 5.915-1.567 11.308-1.218 17.398-2.784 2.783-.696 5.567-1.566 8.35-2.436v-1.565c-3.13-3.132-5.392-7.307-8.698-10.265-8.873-7.657-18.617-15.137-28.707-21.4-5.394-3.48-12.354-5.742-18.095-8.699-2.086-1.045-5.567-1.566-6.784-3.306-3.133-3.827-4.873-8.872-7.134-13.396-5.044-9.57-9.917-20.182-14.267-30.272-3.13-6.786-5.044-13.572-8.872-19.834-17.92-29.577-37.406-47.497-67.33-65.07-6.438-3.653-14.093-5.219-22.27-7.132-4.348-.175-8.699-.522-13.048-.697-2.784-1.218-5.568-4.523-8.004-6.089-9.915-6.264-35.492-19.833-42.799-1.913-4.698 11.308 6.96 22.442 10.96 28.185 2.96 4.001 6.786 8.524 8.874 13.048 1.218 2.956 1.565 6.09 2.783 9.221 2.785 7.653 5.393 16.18 9.048 23.314 1.914 3.653 4.001 7.48 6.437 10.786 1.392 1.913 3.827 2.784 4.35 5.915-2.435 3.48-2.61 8.7-4.003 13.049-6.263 19.66-3.826 44.017 5.046 58.457 2.783 4.348 9.395 13.92 18.268 10.265 7.83-3.131 6.09-13.048 8.35-21.747.524-2.09.176-3.48 1.219-4.872v.349c2.436 4.87 4.871 9.569 7.133 14.44 5.394 8.524 14.788 17.398 22.617 23.314 4.177 3.13 7.482 8.524 12.702 10.438v-.523h-.349c-1.044-1.566-2.61-2.261-4.001-3.48-3.131-3.13-6.612-6.958-9.047-10.438-7.306-9.744-13.745-20.53-19.486-31.665-2.783-5.392-5.22-11.308-7.481-16.701-1.045-2.09-1.045-5.22-2.784-6.263-2.61 3.827-6.437 7.133-8.351 11.83-3.304 7.481-3.653 16.702-4.871 26.27-.696.176-.349 0-.697.35-5.566-1.394-7.48-7.134-9.569-12.006-5.22-12.352-6.09-32.186-1.565-46.452 1.218-3.654 6.438-15.136 4.35-18.616-1.044-3.306-4.525-5.22-6.438-7.829-2.261-3.306-4.698-7.48-6.263-11.135-4.176-9.743-6.264-20.53-10.787-30.273-2.088-4.524-5.74-9.22-8.699-13.396-3.305-4.697-6.959-8.004-9.569-13.571-.869-1.913-2.088-5.045-.696-7.133.348-1.392 1.043-1.913 2.436-2.261 2.262-1.915 8.7.521 10.96 1.565 6.438 2.608 11.831 5.046 17.225 8.699 2.435 1.74 5.045 5.046 8.176 5.916h3.654c5.568 1.217 11.83.348 17.05 1.913 9.222 2.957 17.572 7.307 25.054 12.005 22.792 14.44 41.58 34.97 54.282 59.501 2.088 4 2.957 7.656 4.871 11.83 3.655 8.526 8.178 17.225 11.83 25.576 3.654 8.176 7.133 16.528 12.353 23.314 2.61 3.652 13.048 5.567 17.746 7.481 3.48 1.565 8.874 2.958 12.005 4.871 5.915 3.652 11.83 7.83 17.398 11.83 2.784 2.088 11.482 6.438 12.005 9.917z"/><path d="m58.186 43.022c-2.957 0-5.044.35-7.132.871v.348h.348c1.393 2.784 3.827 4.698 5.566 7.133 1.393 2.783 2.61 5.568 4.003 8.352.173-.175.347-.348.347-.348 2.437-1.741 3.654-4.524 3.654-8.7-1.044-1.217-1.218-2.435-2.088-3.653-1.043-1.741-3.306-2.61-4.698-4.003z"/></g></svg>                    
                    <Text fz={18}>SQL</Text>
                  </Stack>
                </Paper>
              <Paper shadow="xl" c={"black"} p="xl" maw={"200px"}>
                <Stack justify='center' align='center' h={"100%"}>
                  <Image w={"100%"} src="src/assets/cloud-api.png" alt="API"/>
                  <Text fz={18}>API Integrations</Text>
                </Stack>
              </Paper>
            </Flex>
          </Accordion.Panel>
        </Accordion.Item>
        <Accordion.Item key={"Frontend"} value={"Frontend"}>
          <Accordion.Control c={"white"} fz={24}
            style={({
              backgroundColor: 'transparent',
            })}
          >
            Frontend
          </Accordion.Control>
          <Accordion.Panel>
            <Flex gap={24} justify="center" align={isVSm ? 'stretch' : "center"} wrap={"wrap"}>
                <Paper shadow="xl" c="black" p="xl" maw={"200px"}>
                  <Stack justify='center' align='center' h={"100%"}>
                    <svg height="auto" viewBox="175.7 78 490.6 436.9" width="fit" xmlns="http://www.w3.org/2000/svg"><g fill="#61dafb"><path d="m666.3 296.5c0-32.5-40.7-63.3-103.1-82.4 14.4-63.6 8-114.2-20.2-130.4-6.5-3.8-14.1-5.6-22.4-5.6v22.3c4.6 0 8.3.9 11.4 2.6 13.6 7.8 19.5 37.5 14.9 75.7-1.1 9.4-2.9 19.3-5.1 29.4-19.6-4.8-41-8.5-63.5-10.9-13.5-18.5-27.5-35.3-41.6-50 32.6-30.3 63.2-46.9 84-46.9v-22.3c-27.5 0-63.5 19.6-99.9 53.6-36.4-33.8-72.4-53.2-99.9-53.2v22.3c20.7 0 51.4 16.5 84 46.6-14 14.7-28 31.4-41.3 49.9-22.6 2.4-44 6.1-63.6 11-2.3-10-4-19.7-5.2-29-4.7-38.2 1.1-67.9 14.6-75.8 3-1.8 6.9-2.6 11.5-2.6v-22.3c-8.4 0-16 1.8-22.6 5.6-28.1 16.2-34.4 66.7-19.9 130.1-62.2 19.2-102.7 49.9-102.7 82.3 0 32.5 40.7 63.3 103.1 82.4-14.4 63.6-8 114.2 20.2 130.4 6.5 3.8 14.1 5.6 22.5 5.6 27.5 0 63.5-19.6 99.9-53.6 36.4 33.8 72.4 53.2 99.9 53.2 8.4 0 16-1.8 22.6-5.6 28.1-16.2 34.4-66.7 19.9-130.1 62-19.1 102.5-49.9 102.5-82.3zm-130.2-66.7c-3.7 12.9-8.3 26.2-13.5 39.5-4.1-8-8.4-16-13.1-24-4.6-8-9.5-15.8-14.4-23.4 14.2 2.1 27.9 4.7 41 7.9zm-45.8 106.5c-7.8 13.5-15.8 26.3-24.1 38.2-14.9 1.3-30 2-45.2 2-15.1 0-30.2-.7-45-1.9-8.3-11.9-16.4-24.6-24.2-38-7.6-13.1-14.5-26.4-20.8-39.8 6.2-13.4 13.2-26.8 20.7-39.9 7.8-13.5 15.8-26.3 24.1-38.2 14.9-1.3 30-2 45.2-2 15.1 0 30.2.7 45 1.9 8.3 11.9 16.4 24.6 24.2 38 7.6 13.1 14.5 26.4 20.8 39.8-6.3 13.4-13.2 26.8-20.7 39.9zm32.3-13c5.4 13.4 10 26.8 13.8 39.8-13.1 3.2-26.9 5.9-41.2 8 4.9-7.7 9.8-15.6 14.4-23.7 4.6-8 8.9-16.1 13-24.1zm-101.4 106.7c-9.3-9.6-18.6-20.3-27.8-32 9 .4 18.2.7 27.5.7 9.4 0 18.7-.2 27.8-.7-9 11.7-18.3 22.4-27.5 32zm-74.4-58.9c-14.2-2.1-27.9-4.7-41-7.9 3.7-12.9 8.3-26.2 13.5-39.5 4.1 8 8.4 16 13.1 24s9.5 15.8 14.4 23.4zm73.9-208.1c9.3 9.6 18.6 20.3 27.8 32-9-.4-18.2-.7-27.5-.7-9.4 0-18.7.2-27.8.7 9-11.7 18.3-22.4 27.5-32zm-74 58.9c-4.9 7.7-9.8 15.6-14.4 23.7-4.6 8-8.9 16-13 24-5.4-13.4-10-26.8-13.8-39.8 13.1-3.1 26.9-5.8 41.2-7.9zm-90.5 125.2c-35.4-15.1-58.3-34.9-58.3-50.6s22.9-35.6 58.3-50.6c8.6-3.7 18-7 27.7-10.1 5.7 19.6 13.2 40 22.5 60.9-9.2 20.8-16.6 41.1-22.2 60.6-9.9-3.1-19.3-6.5-28-10.2zm53.8 142.9c-13.6-7.8-19.5-37.5-14.9-75.7 1.1-9.4 2.9-19.3 5.1-29.4 19.6 4.8 41 8.5 63.5 10.9 13.5 18.5 27.5 35.3 41.6 50-32.6 30.3-63.2 46.9-84 46.9-4.5-.1-8.3-1-11.3-2.7zm237.2-76.2c4.7 38.2-1.1 67.9-14.6 75.8-3 1.8-6.9 2.6-11.5 2.6-20.7 0-51.4-16.5-84-46.6 14-14.7 28-31.4 41.3-49.9 22.6-2.4 44-6.1 63.6-11 2.3 10.1 4.1 19.8 5.2 29.1zm38.5-66.7c-8.6 3.7-18 7-27.7 10.1-5.7-19.6-13.2-40-22.5-60.9 9.2-20.8 16.6-41.1 22.2-60.6 9.9 3.1 19.3 6.5 28.1 10.2 35.4 15.1 58.3 34.9 58.3 50.6-.1 15.7-23 35.6-58.4 50.6z"/><circle cx="420.9" cy="296.5" r="45.7"/></g></svg>
                    <Text fz={18}>React</Text>
                  </Stack>
                </Paper>
                <Paper shadow="xl" c={"black"} p="xl" maw={"200px"}>
                  <Stack justify='center' align='center' h={"100%"}>
                    <Image w={"100%"} src="/src/assets/typescript.png" alt="TypeScript Logo"/>
                    <Text fz={18}>TypeScript</Text>
                  </Stack>
                </Paper>
                <Paper shadow="xl" c="black" p="xl" maw={"200px"}>
                  <Stack justify='center' align='center' h={"100%"}>
                    <svg fill="none" height="auto" width="fit" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 124 141.53"><path d="M10.383 126.892L0 0l124 .255-10.979 126.637-50.553 14.638z" fill="#1b73ba"/><path d="M62.468 129.275V12.085l51.064.17-9.106 104.85z" fill="#1c88c7"/><path d="M100.851 27.064H22.298l2.128 15.318h37.276l-36.68 15.745 2.127 14.808h54.043l-1.958 20.68-18.298 3.575-16.595-4.255-1.277-11.745H27.83l2.042 24.426 32.681 9.106 31.32-9.957 4-47.745H64.765l36.085-14.978z" fill="#fff"/></svg>                   
                    <Text fz={18}>CSS/Preprossessors</Text>
                  </Stack>
                </Paper>
                <Paper shadow="xl" c={"black"} p="xl" maw={"200px"}>
                  <Stack justify='center' align='center' h={"100%"}>
                    <svg height="auto" viewBox=".15 .13 799.7 479.69" width="fit" xmlns="http://www.w3.org/2000/svg"><path d="m400 .13c-106.63 0-173.27 53.3-199.93 159.89 39.99-53.3 86.64-73.28 139.95-59.96 30.42 7.6 52.16 29.67 76.23 54.09 39.2 39.78 84.57 85.82 183.68 85.82 106.62 0 173.27-53.3 199.92-159.9-39.98 53.3-86.63 73.29-139.95 59.97-30.41-7.6-52.15-29.67-76.22-54.09-39.2-39.78-84.58-85.82-183.68-85.82zm-199.93 239.84c-106.62 0-173.27 53.3-199.92 159.9 39.98-53.3 86.63-73.29 139.95-59.96 30.41 7.61 52.15 29.67 76.22 54.08 39.2 39.78 84.58 85.83 183.68 85.83 106.63 0 173.27-53.3 199.93-159.9-39.99 53.3-86.64 73.29-139.95 59.96-30.42-7.59-52.16-29.67-76.23-54.08-39.2-39.78-84.57-85.83-183.68-85.83z" fill="#06b6d4"/></svg>
                    <Text fz={18}>Tailwind</Text>
                  </Stack>
                </Paper>
                <Paper shadow="xl" c={"black"} p="xl" maw={"200px"}>
                  <Stack justify='center' align='center' h={"100%"}>
                    <Image w={"100%"} src="/src/assets/figma.png" alt="Figma Logo"/>
                    <Text fz={18}>Figma/Design Execution</Text>
                  </Stack>
                </Paper>
                <Paper shadow="xl" c={"black"} p="xl" maw={"200px"}>
                  <Stack justify='center' align='center' h={"100%"}>
                    <Image w={"100%"} src="/src/assets/mantine-seeklogo.png" alt="Mantine Logo"/>
                    <Text fz={18}>Mantine/Component Libraries</Text>
                  </Stack>
                </Paper>
            </Flex>
          </Accordion.Panel>
        </Accordion.Item>
        <Accordion.Item key={"DevOps"} value={"DevOps"}>
        <Accordion.Control c={"white"} fz={24} fw={"900"}
            style={({
              backgroundColor: 'transparent',
            })}
          >
            DevOps
          </Accordion.Control>
          <Accordion.Panel>
            <Flex gap={24} justify="center" align={isVSm ? 'stretch' : "center"} wrap={"wrap"} direction={{ base: 'row', sm: 'row' }}>
              <Paper shadow="xl" c={"black"} p="xl" maw={"200px"}>
                <Stack justify='center' align='center' h={"100%"}>
                  <Image w={"100%"} src="/src/assets/docker.png" alt="Docker Logo"/>
                  <Text fz={18}>Docker/Docker Compose</Text>
                </Stack>
              </Paper>
              <Paper shadow="xl" c={"black"} p="xl" maw={"200px"}>
                <Stack justify='center' align='center' h={"100%"}>
                  <Image w={"100%"} src="src/assets/github-actions-logo-512.png" alt="GitHub Actions Logo"/>
                  <Text fz={18}>GitHub Actions</Text>
                </Stack>
              </Paper>
              <Paper shadow="xl" c={"black"} p="xl" maw={"200px"}>
                <Stack justify='center' align='center' h={"100%"}>
                  <Image w={"100%"} src="src/assets/EC2.png" alt="AWS EC2 Logo"/>
                  <Text fz={18}>AWS EC2</Text>
                </Stack>
              </Paper>
              <Paper shadow="xl" c={"black"} p="xl" maw={"200px"}>
                <Stack justify='center' align='center' h={"100%"}>
                  <Image w={"100%"} src="/src/assets/Simple Storage Service.png" alt="AWS S3 Logo"/>
                  <Text fz={18}>AWS S3</Text>
                </Stack>
              </Paper>
              <Paper shadow="xl" c={"black"} p="xl" maw={"200px"}>
                <Stack justify='center' align='center' h={"100%"}>
                  <Image w={"100%"} src="src/assets/CodePipeline.png" alt="AWS CodePipeline Logo"/>
                  <Text fz={18}>AWS CodePipeline</Text>
                </Stack>
              </Paper>
              <Paper shadow="xl" c={"black"} p="xl" maw={"200px"}>
                <Stack justify='center' align='center' h={"100%"}>
                  <Image w={"100%"} src="src/assets/CodeDeploy.png" alt="AWS CodeDeploy Logo"/>
                  <Text fz={18}>AWS CodeDeploy</Text>
                </Stack>
              </Paper>
            </Flex>
          </Accordion.Panel>
        </Accordion.Item>
      </Accordion>
    );
  }